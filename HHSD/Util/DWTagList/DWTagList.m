...
- (void)setOneLineTags:(NSArray *)array
{
    if(array.count >3)
    {
        textArray = [NSArray arrayWithObjects:array[0],array[1],array[3], nil];
    }else
    {
        textArray = [[NSArray alloc] initWithArray:array];
    }
...
//        __block NSMutableString *tmpString = [NSMutableString string];
        [textArray enumerateObjectsUsingBlock:^(id  _Nonnull text, NSUInteger idx, BOOL * _Nonnull stop) {
            DWTagView *tagView;
            if (tagViews.count > 0) {
                tagView = [tagViews lastObject];
                [tagViews removeLastObject];
...
//    self.sizeToFit  sizeFit;
//    self.frame.size = sizeFit;
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, sizeFit.width, sizeFit.height);

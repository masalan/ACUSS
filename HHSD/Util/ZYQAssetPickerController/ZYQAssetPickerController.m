...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==ceil(self.assets.count*1.0/columns)) {
        ...
        if (_numberOfVideos == 0)
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld Photo", nil), (long)_numberOfPhotos];
        else if (_numberOfPhotos == 0)
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld Video", nil), (long)_numberOfVideos];
        else
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (photosSelected && videoSelected)
        format = NSLocalizedString(@"Already selecte %ld Items", nil);
    
    else if (photosSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@" %ld photo", nil) : NSLocalizedString(@" %ld Photo ", nil);
    
    else if (videoSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@" %ld video", nil) : NSLocalizedString(@" %ld Video ", nil);
...
- (NSString *)accessibilityLabel
{
    ...
    return [label stringByAppendingFormat:NSLocalizedString(@"%ld Photo", nil), (long)[self.assetsGroup numberOfAssets]];
}
...
- (void)viewDidLoad
{
    ...
    [self.navigationController.navigationBar setBarTintColor:KTHEME_COLOR];
...
- (void)showNotAllowed
{
    ...
    title.text              = NSLocalizedString(@"This app can not use your photos or videos. ", nil);
    title.font              = [UIFont boldSystemFontOfSize:17.0];
    title.textColor         = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedString(@"You can enable access in the Privacy Settings in.", nil);
...
- (void)showNoAssets
{
    ...
    title.text              = NSLocalizedString(@"No Photo or Video", nil);
    title.font              = [UIFont systemFontOfSize:26.0];
    title.textColor         = [UIColor colorWithRed:153.0/255.0 green:153.0/255.0 blue:153.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedString(@"You can use iTunes to photos and videos\n Synced to iPhone。", nil);

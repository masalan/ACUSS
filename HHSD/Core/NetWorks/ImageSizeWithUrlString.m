...
#import "ImageSizeWithUrlString.h"

@implementation ImageSizeWithUrlString
+(CGSize)downloadImageSizeWithURLString:(id)imageURL
{
    ...
    if([[SDImageCache sharedImageCache] diskImageExistsWithKey:absoluteString])
    {
        ...
        if(!image)
        {
            NSData* data = [[SDImageCache sharedImageCache] performSelector:@selector(diskImageDataBySearchingAllPathsForKey:)
                                                                 withObject:URL.absoluteString];
            image = [UIImage imageWithData:data];
            CGSize size = image.size;
            size = [self SizeLow:size];
            return size;
        }else
        {
            CGSize size = image.size;
...
{
    if(size.height>180 || size.width >180)
    {
        size = [self SizeLow:CGSizeMake(size.width/3*2, size.height/3*2)];
    }

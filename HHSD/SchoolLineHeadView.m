...
@implementation SchoolLineHeadView
- (instancetype)initWithMode:(School_Details *)mode
{
    ...
    if([mode.photo hasPrefix:@"http"]) {
        [_logoSchool sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",mode.photo]]
    ...
    _nameUniversity = [UILabel createLabelWithFrame:CGRectMake(_logoSchool.right+10, 10, SCREEN_WIDTH-(logo_length+10)-20,50)
                                backgroundColor:KCOLOR_CLEAR
                                  ...
                                  textalignment:NSTextAlignmentCenter
                                           text:mode.nameSchool];
    if (mode.nameSchool) {
        _nameUniversity.text = [NSString stringWithFormat:@"%@",mode.nameSchool];
    }else{
    ...
    if (mode.locationName) {
        _location.text = [NSString stringWithFormat:NSLocalizedString(@"Localized_SchoolLineHeadView_location",comment:""),mode.locationName];
    }else{
    ...
    NSArray *array = [NSArray arrayWithObjects:NSLocalizedString(@"Localized_SchoolLineHeadView_bachelor",comment:""),NSLocalizedString(@"Localized_SchoolLineHeadView_master",comment:""),NSLocalizedString(@"Localized_SchoolLineHeadView_Phd",comment:""),  nil];

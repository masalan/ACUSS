...
#define kfontColor  KCOLOR_Black_576b95
...
- (void)drawRect:(CGRect)rect {
	...
	for (int i = 0; i < _numOfV; i++) {
        ...
        NSString *scoreName = _scoreArray[i];
        sumScore = [scoreName intValue] + sumScore;
...
//        if (__IPHONE_OS_VERSION_MIN_REQUIRED >= 70000) {
        ...
        NSDictionary *attributes = @{ NSFontAttributeName: self.scaleFont,NSForegroundColorAttributeName:KCOLOR_Black_576b95,
...
//        [[NSString stringWithFormat:@"%@分",scoreName] drawInRect:CGRectMake(legendCenter.x - width / 2.0,
//                                             legendCenter.y - height / 2.0 + 13,
//                                             width,
//                                             height)
//                   withAttributes:attributes];
...
//	}
    ...
    [paragraphStyle setLineBreakMode:NSLineBreakByClipping];
    [paragraphStyle setAlignment:NSTextAlignmentCenter];
    NSDictionary *attributes = @{ NSFontAttributeName: KSYSTEM_FONT_(100),NSForegroundColorAttributeName:KCOLOR_THEME,
                                  NSParagraphStyleAttributeName: paragraphStyle };
    NSString *scoreString = [NSString stringWithFormat:@"%d ",(int)_sum_Score];
    //[scoreString drawAtPoint:CGPointMake(_centerPoint.x- 20, _centerPoint.y - 12) withAttributes:attributes];
   //[scoreString drawAtPoint:CGPointMake((_centerPoint.x)-58, _centerPoint.y) withAttributes:attributes];
    [scoreString drawAtPoint:CGPointMake((SCREEN_WIDTH/2.0)-100, (_centerPoint.y)-50.0) withAttributes:attributes];


//    [[NSString stringWithFormat:@"%d",sumScore] drawInRect:
//                                              withAttributes:attributes];
